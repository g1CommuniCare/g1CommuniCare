...
export default function Account() {
  ...
  async function fetchUpdatedImage() {
    const res = await axios(
      `http://localhost:8080/resident/getResidentById/${residentId}`
    );
    const data = res.data;
  ...
  const currentPageRowsMyReports = myReport.slice(
    startIndexMyReports,
    endIndexMyReports
  );
  const currentPageRowsMyRequests = myRequest.slice(
    startIndexMyRequests,
    endIndexMyRequests
  );
  ...
  return (
    ...
    >
      <>
        ...
        <div className="flex justify-between gap-8 rounded-[22px] w-11/12 p-7 bg-slate-100/70 ml-16 mt-8">
  ...
  return (
    <div className="mt-4 flex justify-center space-x-2">
      ...
      <button
        ...
        className={"flex items-center gap-2 px-6 py-3 font-sans text-xs font-bold text-center text-gray-900 uppercase align-middle transition-all rounded-full select-none hover:bg-emerald-300 active:bg-emerald-300/20 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"}
      >
