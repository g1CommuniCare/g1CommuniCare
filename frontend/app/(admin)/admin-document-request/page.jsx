...
function TableDocumentRequests({
    ...
    async function handleDelete(event, docreqId) {
        ...
        try {
            const response = await fetch(`http://localhost:8080/document-requests/${docreqId}/soft-delete`, {
                method: 'PUT',
            });
            ...
            if (response.ok) {
                ...
                setDocumentRequests(currentDocumentRequests => 
                    currentDocumentRequests.filter(request => request.docreqId !== docreqId)
                );
            } else {
            ...
            <tr onClick={handleClick} className="hover:bg-gray-200 bg-white cursor-pointer">
                ...
                <td className="h-10 text-center px-4 text-sm font-semibold transition duration-300 border-b-2 text-slate-700 border-slate-200">
                    <span 
                        ...
                        borderRadius: '9999px',
                        backgroundColor: documentStatus === 'Approved' ? '#22f200' : // Green shade
                                        documentStatus === 'Denied' ? '#ff6363' : // Red shade
                                        documentStatus === 'Pending' ? '#ff9f2e' : // Yellow shade
                ...
                <td className="h-10 px-2 text-sm font-semibold transition duration-300 border-b-2 border-r-2 text-slate-700 border-slate-200">
                    <button onClick={(event) => handleDelete(event, docreqId)} className="text-red-500 hover:text-red-700">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
